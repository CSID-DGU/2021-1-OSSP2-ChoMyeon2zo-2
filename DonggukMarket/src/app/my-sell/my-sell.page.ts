...
@Component({
  ...
  templateUrl: './my-sell.page.html',
  styleUrls: ['./my-sell.page.scss'],
})
export class MySellPage implements OnInit {
  ...
  ngOnInit() {
    this.stor.get('id').then((val) => {
      this.userid = val;
      firebase.database().ref().child(`userinfo/${this.userid}/trade_list/sell`).once('value', function(data){
        var user = data.val();
